...
#if defined(NDEBUG)
# error "Stakehouse cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x150dc5448aec799fc389b077f1793f6211d3b2467aaa36f0508b9029c330dc66");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Stakehouse: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Stakehouse Signed Message:\n";
...
{
    // Stakehouse: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Stakehouse: 10k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Stakehouse: 1 days
static const int64 nTargetSpacing = 5 * 60; // Stakehouse: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Stakehouse: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Stakehouse: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x20a4bb6f2d982c90f59ecf52b1a6ca56133aefea0d516550c012ecc7b3fd11ae");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Once again it's on. -Ice Cube";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a3a1460c7685a29f69d5f35d17319b6d342342e416dc0cc6dc00313c6915043437605e25e1cd89e1c28f1f0fc7cdd457e264846953c8dc75abbc4fddcff56460") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1542350608;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085024467;

        if (fTestNet)
        {
            block.nTime    = 1542350579;
            block.nNonce   = 386615116;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x078e4e92fd0c9aff18d3153cd4e2779fe805115f8c726d8ced0e39d104e409af"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xca, 0xb4, 0xdc }; // Stakehouse: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("StakehouseMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("StakehouseMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static StakehouseMiner(CWallet *pwallet)
{
    printf("StakehouseMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("stakehouse-miner");
    ...
    try { loop {
        ...
        printf("Running StakehouseMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("StakehouseMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&StakehouseMiner, pwallet));
}
